...
from explainer import BookPreferences, generate_explanations
from logger import get_logger
from vectordb import get_book_vector_matches, search_by_vector
...
def _apply_filters(query, preferences):
    ...
    if preferences.age is not None:
        ...
        query = query.filter(
            (
                (func.instr(Book.age_range, "-") > 0)
                & (cast(func.substr(Book.age_range, 1, func.instr(Book.age_range, "-") - 1), Integer) <= age)
                & (cast(func.substr(Book.age_range, func.instr(Book.age_range, "-") + 1), Integer) >= age)
            )
            | ((func.instr(Book.age_range, "-") == 0) & (cast(Book.age_range, Integer) == age))
        )
...
def search_books(db: Session, preferences: BookPreferences) -> List[Dict[str, Any]]:
    ...
    Workflow:
    1. If preferences.query_text is provided, perform a semantic vector search to get relevant book IDs.
    ...
    5. If the SQL results are insufficient, supplement with vector search results (avoiding duplicates).
    ...
    logger.debug(
        f"SQL filters: age={preferences.age}, purpose={preferences.purpose}, genre={preferences.genre}, "
        f"budget={preferences.budget}, query_text={preferences.query_text}"
    )
    ...
    logger.info(
        ...
        f"genre={preferences.genre}, query_text={preferences.query_text and 'provided' or 'none'}"
    )
    ...
    if books:
        ...
        for book in books:
            ...
            book_dict["explanation"] = explanations.get(
                int(book.id), f"This book is suitable for {book.age_range} age range and focuses on {book.purpose}."
            )
